	...
	this.paint = function(navigationDir) {
		...
		rect
			...
			.on("click", function(d) {
				...
				var _positionX = self.positionSubDomainX(d.t) + options.cellSize + 5;
				var _positionY = self.positionSubDomainY(d.t) + options.cellSize/2;

				// Offset by the domain position
				_positionX += parseInt(domainNode.getAttribute("x"), 10);
				_positionY += parseInt(domainNode.getAttribute("y"), 10);

				// Offset by the calendar position (when legend is left/top)
				_positionX += parseInt(self.root.select(".graph").attr("x"), 10);
				_positionY += parseInt(self.root.select(".graph").attr("y"), 10);

				// Offset by the inside domain position (when label is left/top)
				_positionX += parseInt(domainNode.parentNode.getAttribute("x"), 10);
				_positionY += parseInt(domainNode.parentNode.getAttribute("y"), 10);
				if (options.onClick !== null) {
					return self.onClick(new Date(d.t), d.v, { x: _positionX, y: _positionY});
				}
			})
...
CalHeatMap.prototype = {
	 ...
	 */
	onClick: function(d, itemNb, elPosition) {
		"use strict";

		return this.triggerEvent("onClick", [d, itemNb, elPosition]);
	},
